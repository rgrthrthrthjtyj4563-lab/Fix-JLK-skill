...
| 编号 | 测试项 | 命令 | 预期结果 |
|------|--------|------|----------|
| T2.1 | 正常加载 | `python3 -c "from scripts.build_payload import load_dimension_library; lib=load_dimension_library(); print(type(lib), len(lib['themes']))"` | `<class 'dict'> 2` |
...
| T2.4 | 主题名列表 | `python3 -c "from scripts.build_payload import load_dimension_library; lib=load_dimension_library(); print([t['theme_name'] for t in lib['themes']])"` | `['心达康胶囊的患者依从性调研问卷', '心达康胶囊-药品可及性与价格敏感度调查（患者）']` |
