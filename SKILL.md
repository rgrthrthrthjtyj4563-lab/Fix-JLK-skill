...
4. 用 `scripts/build_payload.py` 构建 `report_payload.json`。如果 AI 草稿中的 `4.x` 章节集合、标题或小标题与 `cluster_dimensions` 骨架冲突，必须直接报错，不做兼容。
   ...
   - 第二张概览图（PNG 柱状图）不得包含 `值` 图例；`ax.legend()` 调用必须移除。
   - `5.1 问卷重点问题分析` 必须保留 `2` 张原生 `3D` 饼图，并与第 `4` 部分的两张概览图并存，不能因为修第 `4` 部分图表而删除 `5.1` 双图。
   - `5.1 问卷重点问题分析` 的图文顺序必须按"分析段落 → 对应图表"交错排列：第一段分析后紧跟第一张饼图，第二段分析后紧跟第二张饼图，不得将两张图表集中插入标题之后。
   - `5.1` 原生 `3D` 饼图的内嵌 workbook 数据必须与 chart XML 缓存数据一致（分类标签、数值、`formatCode="0.00%"`），确保 Word 右键编辑图表时不会覆写出错误数据。
   - `问卷调研服务结算` 必须按样本量实际计算：样本费用 = 样本数 × 100，报告费用 = 30000，总计 = 样本费用 + 报告费用。
   - 日期展示格式强制使用紧凑格式：`2025年05月13日-05月31日`（起止日期以 `-` 连接，同年省略结束年份）。内部存储仍保留完整格式用于计算（如 `service_date` 推导），但所有渲染文本必须使用 `format_survey_period_display()` 输出。
   - 封面字段"项目名称"必须替换为"服务商"标签，取值与 `service.unit` 一致。
   - `service.unit` 和 `disclaimer.unit` 必须强制同一来源，`validate_payload` 校验不一致时直接报错。
   - `None/nan/null` 不得出现在最终交付文档中；`final_validate_docx.py` 增加兜底校验，发现立即阻断交付。
