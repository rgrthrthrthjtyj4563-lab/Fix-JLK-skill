...
class TemplateRenderer:
    ...
    def replace_settlement(self):
        ...
        def set_cell_text(cells: list, index: int, text: str) -> None:
            if len(cells) <= index:
                return
            for p in cells[index].findall(qn("w:p")):
        ...
        if len(rows) >= 2:
            ...
            set_cell_text(cells, 4, format_amount(settlement.get("sample_amount")))
        if len(rows) >= 3:
            cells = rows[2].findall(qn("w:tc"))
            set_cell_text(cells, 4, format_amount(settlement.get("report_amount")))
        if len(rows) >= 4:
